import Repository from "../../AdminRepository";
import qs from "qs";

const resource = "subjects";
const headers = {
  'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6Ik5UZG1aak00WkRrM05qWTBZemM1TW1abU9EZ3dNVEUzTVdZd05ERTVNV1JsWkRnNE56YzRaQT09In0..nCLEp_Bo5qk18JiuEJ84QL_I-RpC2cXoCO-NLNyX4Um7xSUXDukvzjVY8qPIJftHaAB5Tl1Zs2MML_5kRAeXsLJIbpOSdWQBoYx_KvMDTvkPqSZBcrDpDdihCjCWI3au8vLLOptcYwE0knzlbu3NFT5Q1UYhtJRuZkNS4mlqvZ3Lr2GxNQJbRacWH73Vcso7CMhztIZTxIPdczncCYWnKsuYimllXSvc-ICxHYYnb5uQ2Cf-s7kprQrhGbvsAtvQHYQzxJBCfo1bMANdk2OCxrfrWp459Vq9XEK7iqjsHTEWmoNQkXa8uF9vflmIXzJBTchLayFQLT9uGmetpm-6yQ'
};

export default {
  get(id) {
    return Repository.get(`${resource}/${id}.json`);
  },
  getList(searchParams) {
    const paramsSerializer = (params = {}) =>
      qs.stringify(searchParams, { arrayFormat: "brackets" });

    const params = {
      q: this.q,
      page: this.page,
    };

    return Repository.get(`${resource}.json`, {
      params,
      paramsSerializer,
    });
  },
  getListDepartmentOption() {
    return Repository.get(`${resource}/get_list_department_option.json`);
  },
  getListSubjectTypeOption() {
    return Repository.get(`${resource}/get_list_subject_type_option.json`);
  },
  getListNameOption() {
    return Repository.get(`${resource}/get_list_name_option.json`);
  },
  getListNameCodeOption() {
    return Repository.get(`${resource}/get_list_name_code_option.json`)
  },
  create(params) {
    return Repository.post(
      `https://localhost:8243/create_subject/1.0?subject_code=${params.subject_code}&subject_name=${params.subject_name}&subject_type=${params.subject_type}&credit_value=${params.credit_value}&jhi_desc=${params.jhi_desc}&department=${params.department}&status=${params.status}&credit_value_number=${params.credit_value_number}&require_pair=${params.require_pair}&subject_required=${params.subject_required}`, params, {
        headers: headers
    });
  },
  update(params) {
    return Repository.put(`${resource}/${params.id}.json`, params);
  },
  destroy(id) {
    return Repository.delete(`${resource}/${id}.json`);
  }
};
